...
const Header = ({ darkMode, setDarkMode }) => {
  ...
  const contact = [
    { to: profile.contact.linkedin, label: "LinkedIn" },
    { to: profile.contact.github, label: "GitHub" },
    { to: `mailto:${profile.contact.email}`, label: "Email" },
  ];
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? "glass-card border-b border-gray-100 dark:border-gray-800"
          : ""
      }`}
    >
      <nav className="container mx-auto px-6 py-3 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center space-x-6">
          <p className="hidden md:hidden">|</p>
          {contact.map((link) => (
            <a
              key={link.to}
              href={link.to}
              target={link.to.startsWith("mailto:") ? "_self" : "_blank"}
